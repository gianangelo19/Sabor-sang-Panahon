...
func _run() -> void:
	var map := (load("res://game/worlds/la_paz/la_paz.tscn") as PackedScene).instantiate()
	var street_player := map.get_node("StreetLapazAmbience")
	_check(
		street_player.get("audio_path") == "res://assets/audio/ambience/la_paz_street_loop.ogg",
		"The map uses the new global street ambience",
	)

	var market := (load("res://game/worlds/la_paz/public_market/lapaz_public_market.tscn") as PackedScene).instantiate()
	var market_player := market.get_node("MarketAmbience") as AudioStreamPlayer3D
	_check(
		market_player.get("audio_path") == "res://assets/audio/ambience/la_paz_public_market_loop.ogg",
	...
	_check(
		home_player.get("audio_path") == "res://assets/audio/ambience/grandmas_house_loop.ogg",
	...
	_check(
		apartment_player.get("audio_path") == "res://assets/audio/ambience/apartment_room_loop.ogg",
		"The apartment uses its dedicated room ambience",
	)

	var loop_player := AudioStreamPlayer.new()
	loop_player.set_script(load("res://core/audio/looping_audio_player.gd"))
	loop_player.set("audio_path", "res://assets/audio/ambience/la_paz_street_loop.ogg")
