...
func _run() -> void:
	...
	_check(
		not hud.inventory_ui.hotbar_panel.visible,
		"Hotbar stays hidden before the wake-up tutorial begins",
	)
	_check(hud.ingredient_label.text == "0/7 collected", "HUD uses the seven-ingredient story total")
	hud._on_ingredients_changed(7, 7)
	_check(hud.ingredient_label.text == "7/7 collected", "HUD clearly marks all seven ingredients collected")
	...
	hud._on_ingredients_changed(0, 7)
	...
	_check(
		not hud.inventory_ui.hotbar_panel.visible,
		"Hotbar does not cover the movement instruction",
	)
	...
	_check(game_state.tutorial_step == hud.TUTORIAL_WAITING_FOR_FRIDGE, "Movement and jump completion starts the fridge wait")
	_check(not hud.hint_bar.visible, "No new instruction appears before the fridge is seen")

	hud.notify_fridge_seen()
	await create_timer(0.35).timeout
	_check(game_state.tutorial_step == hud.TUTORIAL_INTERACT, "Seeing the fridge unlocks interaction instruction")
	_check(hud.hint_text.text == "INTERACT", "The first interaction lesson stays generic")
	_check(hud.primary_key.texture.resource_path.ends_with("key_f.png"), "Interaction instruction uses the F sprite")
	_check(
		not hud.inventory_ui.hotbar_panel.visible,
		"Hotbar remains hidden until the box reward is acquired",
	)
	...
	_check(
		hud.inventory_ui.hotbar_panel.visible,
		"Hotbar appears after the box reward is acquired",
	)
	var hint_rect: Rect2 = hud.hint_bar.get_global_rect()
	var hotbar_rect: Rect2 = hud.inventory_ui.hotbar_panel.get_global_rect()
	_check(
		hint_rect.end.y <= hotbar_rect.position.y - 10.0,
		"Phone instruction sits above the hotbar with a clear gap",
	)
