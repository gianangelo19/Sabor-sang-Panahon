...
func _run() -> void:
	...
	await _verify_conversation(vendor_1, "Market Vendor", "npc_market_vendor_front.png", 10, "fresh_miki", "Fresh miki noodles")
	...
	_check(game_state.clues.has("Market testimony: fresh miki feels connected, but the vendor cannot remember the dish."), "First vendor records the forgotten-dish clue")

	await _verify_conversation(vendor_2, "Market Vendor 2", "npc_market_vendor2_front.png", 10, "pork_and_liver", "Pork and liver")
	...
	_check(game_state.clues.has("Market testimony: muscle memory connects pork and liver to the forgotten bowl."), "Second vendor records the memory-gap clue")

	await _verify_conversation(chicharon_vendor, "Chicharon Vendor", "npc_chicharon_vendor_front.png", 10, "crushed_chicharon", "Crushed chicharon")
	...
	_check(game_state.current_objective == "Talk to the tindero for the egg.", "Chicharon vendor points to the egg vendor")

	await _verify_conversation(tindero, "Tindero", "npc_tindero_front.png", 6, "egg", "Fresh egg")
	...
	_check(game_state.clues.has("Tindero testimony: a fresh egg may complete the remembered bowl."), "Tindero records the egg clue")
	_check(game_state.current_objective == "Review the gathered food clues with AMBot.", "Final vendor advances the investigation")
...
func _verify_conversation(npc: Node, npc_name: String, portrait_filename: String, expected_lines: int, reward_id: String, reward_name: String) -> void:
	await process_frame
	...
	var game_state := root.get_node("GameState")
	_check(minigame != null, npc_name + " opens a minigame placeholder after dialogue")
	_check(not game_state.has_ingredient(reward_id), npc_name + " does not award its ingredient before a win")
	if minigame:
		_check(minigame.reward_label.text == "Reward: " + reward_name, npc_name + " shows the correct ingredient reward")
		minigame._on_complete_pressed()
		await process_frame
